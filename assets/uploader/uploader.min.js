		...
		var uploader = {
			...
			validate_minimum_file_size : function(input_el) {
				if (config.minimum_file_size !== null) {
	            	var arr_file_name = []
	            	for (var i = 0; i < input_el.length; i++) {
	            		if (input_el[i].size < config.minimum_file_size) {
	            			arr_file_name.push(input_el[i].name)
	            		}
	            	}
                        ...
                        if (xhr.upload) {
                            xhr.upload.addEventListener('progress', function(event) {
                                ...
                                if (percent < 100) {
                                    ...
                                    $(template).find('.uploaderbox-filesize').html(percent + "%")
                                } else {
		...
		var filepicker = {
			...
			on_change: function() {
				$('#' + filepicker.input_file).change(function() {
					...
					var validate_upload_minimum_file_size = filepicker.validate_upload_minimum_file_size(input_el)
    				var validate_upload_file_type = filepicker.validate_upload_file_type(input_el)

    				if (validate_upload_maximum_file_size && validate_upload_minimum_file_size && validate_upload_file_type) {
